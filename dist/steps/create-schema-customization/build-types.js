...
const interfaceType = typeBuilderApi => {
  ...
  interfaces.find(singleInterface => singleInterface.name === type.name)).map(type => typeMap.get(type.name)).filter(type => type.kind !== `UNION` || !!type.possibleTypes && !!type.possibleTypes.length);
...
const objectType = typeBuilderApi => {
  ...
  type !== null && type !== void 0 && (_type$interfaces = type.interfaces) !== null && _type$interfaces !== void 0 && _type$interfaces.find(({
    name
  }) => name === `Node`)) {
